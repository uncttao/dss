...
data LTEWidthV  : (w1, w2 : WidthV) -> (intent: CheckWidthIntent) -> Type where
  ...
  ValueLTE      : (intent: CheckWidthIntent) -> (LTE n m) -> LTEWidthV (Px n) (Px m) intent
...
isLTEWidthV (Px m) (Px n) intent = case (isLTE m n) of
  ...
  No notMAtMostN => No (\(ValueLTE intent mAtMostN) => notMAtMostN mAtMostN)
...
reflectWithError: String -> TT -> TT -> Maybe (List ErrorReportPart)
reflectWithError "LeftBounded" w1 w2 = Just [TextPart "You either specified a width ", TermPart w2, TextPart " smaller than min-width ", TermPart w1, TextPart " or a min-width", TermPart w1, TextPart " greater than width ", TermPart w2, TextPart "!"]
reflectWithError "RightBounded" w1 w2 = Just [TextPart "Hey, it doesn't make sense to say width is ", TermPart w1, TextPart " but having a max-width of ", TermPart w2, TextPart "!"]
reflectWithError "RangeSound" w1 w2 = Just [TextPart "Oops! min-width", TermPart w1, TextPart " cannot be greater than max-width ", TermPart w2, TextPart ". Please try again!"]
reflectWithError _ _ _ = Nothing

%error_handler
widthErr : Err -> Maybe (List ErrorReportPart)
widthErr (CantSolveGoal `(LTEWidthV ~w1 ~w2 ~intent) _) = case intent of
  (P _ (NS (UN message) _) _)  => reflectWithError message w1 w2
widthErr _ = Nothing
